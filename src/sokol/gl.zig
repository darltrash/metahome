...
pub fn getError() callconv(.Inline) Error {
    return sgl_error();
}
pub extern fn sgl_defaults() void;
pub fn defaults() callconv(.Inline) void {
    sgl_defaults();
}
pub extern fn sgl_rad(f32) f32;
pub fn asRadians(deg: f32) callconv(.Inline) f32 {
    return sgl_rad(deg);
}
pub extern fn sgl_deg(f32) f32;
pub fn asDegrees(rad: f32) callconv(.Inline) f32 {
    return sgl_deg(rad);
}
pub extern fn sgl_make_pipeline([*c]const sg.PipelineDesc) Pipeline;
pub fn makePipeline(desc: sg.PipelineDesc) callconv(.Inline) Pipeline {
